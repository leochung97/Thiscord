...
const DirectMessage = (props) => {
  ...
  useEffect(() => {
    if (edit) {
      const messageContainer = document.querySelector(
        `#directMessage-${props.direct_message.id}`
      );
    ...
    if (props.currentUser.id === props.direct_message.user.id) {
      return (
        <div className="message-button-shell">
          ...
          <div
            onClick={() => props.deleteDirectMessage(props.direct_message.id)}
          >
            <svg aria-hidden="false" width="16" height="16" viewBox="0 0 24 24">
              <path
                fill="currentColor"
    ...
    return (
      <div className="channel-message-edit">
        <form
          ...
          className="channel-message-edit-form"
        >
          <input
            type="text"
            value={content}
          ...
          <div className="message-edit-footer">
            ...
            • enter to <span onClick={(e) => handleSubmit(e)}>save</span>
          </div>
        </form>
      </div>
    );
  ...
  return <div className="message-body">{bodyContent()}</div>;
};
